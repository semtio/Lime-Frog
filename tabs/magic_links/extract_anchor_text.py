...
# Формат input.txt (по одной паре на строку):
#   https://site.com/page1 https://site.com/target-link
# Тест:
#   https://example.com https://example.com/more
# Ожидаем: если на странице есть <a href="/more">…</a>, то текст ссылки попадает в CSV.
# Параметры:
...
def find_anchor_text(html: str, base_url: str, target_url: str) -> Optional[str]:
    ...
    soft_matches: List[Tuple[str, str]] = []
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        text = _clean_anchor_text(anchor.get_text(separator=" ", strip=True))

        if resolved_href == target_absolute:
            direct_matches.append((resolved_href, text))
            continue

        candidate_canon = canonicalize_url(resolved_href)
        if candidate_canon == target_canonical:
            soft_matches.append((resolved_href, text))

    for candidates in (direct_matches, soft_matches):
        if not candidates:
            continue
        for _, text in candidates:
            if text:
                return text
        longest_text = max((text for _, text in candidates), key=len, default="")
        if longest_text:
            return longest_text
    return None
...
def process_line(
    ...
    html = response.text
    try:
        anchor_text = find_anchor_text(html, response.url or first_url, second_url)
    except Exception:
    ...
    return ProcessResult(first_url, second_url, anchor_text, "ok")
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract anchor text for paired URLs")
    parser.add_argument(
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    with open(args.output_path, "w", encoding="utf-8", newline="") as csvfile:
        ...
        writer.writerow(["first_url", "second_url", "anchor_text", "status"])
