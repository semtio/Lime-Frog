...
- Создаст виртуальное окружение в текущей папке и установит зависимости
- Настроит Gunicorn (bind на 127.0.0.1:8000)
- Настроит Nginx как reverse proxy (автоопределение свободного порта)
- Откроет необходимые порты в UFW
- Создаст systemd сервис для автозапуска
- Запустит приложение

**Примечание для HestiaCP/VestaCP:** на таких серверах firewall обычно управляется панелью, а не UFW. Скрипт попытается открыть порт через панель автоматически.

**ВАЖНО:** Приложение устанавливается в ту же папку, где вы запустили `install.sh`. После установки можно делать `git pull` для обновления и повторно запускать `sudo ./install.sh` для применения изменений.

После установки приложение будет доступно по адресу, который покажет скрипт:
```
...
**На production сервере НЕ используйте `python start.py` напрямую!**
Приложение работает как systemd сервис:
...
# Запуск
sudo systemctl start seo-checker

# Просмотр логов в реальном времени
sudo journalctl -u seo-checker -f

# Последние 50 строк логов
sudo journalctl -u seo-checker -n 50 --no-pager
```
...
# Запустите install.sh для применения изменений
sudo ./install.sh
```
